  ...
  return (
    <div id="privacy-policy-modal-backdrop" className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div 
        ...
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl text-slate-900 dark:text-white relative animate-in fade-in zoom-in duration-200"
      >
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 text-xs text-slate-700 dark:text-slate-300">
            {policy.introduction}
          </div>
          ...
          <div className="space-y-4">
            {policy.sections.map((section, idx) => (
              <div key={idx} className="space-y-1.5">
                ...
                <p className="text-slate-600 dark:text-slate-300 text-xs sm:text-sm pl-4 leading-relaxed">
                  {section.content}
                </p>
              </div>
            ))}
          ...
          <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/40 flex items-start gap-3 text-xs text-emerald-800 dark:text-emerald-300">
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex justify-end shrink-0">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white text-xs font-semibold border border-slate-800 dark:border-slate-700 transition-colors"
          >
            Entendido y Cerrar
          </button>
        </div>
