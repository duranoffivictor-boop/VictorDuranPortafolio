...
import { ExternalLink, Github, MessageCircle, Layers, CheckCircle } from 'lucide-react';
...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({ projects, whatsappNumber }) => {
  ...
  return (
    <section id="proyectos" className="py-20 bg-slate-900 text-white relative">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredProjects.map((project) => {
            ...
            return (
              <article
                key={project.id}
                className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden hover:border-emerald-500/50 hover:shadow-2xl hover:shadow-emerald-950/40 transition-all flex flex-col group"
              >
                ...
                <div className="relative h-60 w-full overflow-hidden bg-slate-900">
                  ...
                  {/* Category Pill */}
                  <span className="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-mono font-medium bg-slate-900/90 text-emerald-400 border border-slate-700 backdrop-blur-md">
                ...
                <div className="p-6 sm:p-8 flex-1 flex flex-col justify-between space-y-6">
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {project.tags.map((tag, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-0.5 rounded text-[11px] font-mono font-medium bg-slate-900 text-slate-300 border border-slate-800"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="pt-4 border-t border-slate-800/80 flex flex-col sm:flex-row items-center gap-3">
                    <a
                      ...
                      className="w-full sm:flex-1 inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-semibold shadow-md active:scale-95 transition-all"
                    >
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      {project.demoUrl && project.demoUrl !== '#' && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition-colors"
                          title="Ver Demo"
                        >
